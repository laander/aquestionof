...
function nzshpcrt_epay_callback()
{
	...
	if ((isset($_GET['epay_callback']) && $_GET['epay_callback'] == '1') || (isset($_GET['epay_accept']) && $_GET['epay_accept'] == '1')) {
		if (isset($_GET['amount']) && isset($_GET['orderid']) && isset($_GET['tid']) && isset($_GET['cur']) && isset($_GET['cardid'])) {
			...
			$sql = "UPDATE ".WPSC_TABLE_PURCHASE_LOGS." set transactid = " . $_GET['tid'] . ", processed = 2 where id = " . $_GET['orderid'];
			$wpdb->query($sql);	
			...
			if (isset($_GET['epay_callback']) && $_GET['epay_callback'] == '1') {
				$sql = "UPDATE ".WPSC_TABLE_PURCHASE_LOGS." set notes = '" . $purchase_log['notes'] . "\nPayment approved at ePay with transactionid " . $_GET['tid'] . ", cardnopostfix " . $_GET['cardnopostfix'] . ", orderid " . $_GET['orderid'] . ", amount " . $_GET['amount'] . ", currency " . $_GET['cur'] . ", cardtype " . calcCardtype($_GET['cardid']) . "', processed = 2 where id = " . $_GET['orderid'];
				$wpdb->query($sql);	
