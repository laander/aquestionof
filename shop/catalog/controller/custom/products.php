<?php 
class ControllerCustomProducts extends Controller {
	private $error = array();

	// Queries for all products in DB and outputs them as JSON for WP (see view)
...
*/
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/custom/json_products.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/custom/json_products.tpl';
			} else {
